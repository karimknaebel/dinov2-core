...
def _load_state_dict(weights: str | Path, check_hash: bool) -> dict[str, torch.Tensor]:
    ...
    return torch.load(Path(location).expanduser(), map_location="cpu", weights_only=True)
    ...
    **kwargs: Any,
    ...
    if pretrained:
        state_dict = _load_state_dict(
            weights or f"{_DINOV2_BASE_URL}/{model_base_name}/{model_full_name}_pretrain.pth",
            check_hash=check_hash,
        )
...
def dinov2_vits14(
    *, pretrained: bool = True, weights: str | Path | None = None, check_hash: bool = False, **kwargs: Any
):
    return _make_model(
        vit_small,
        "dinov2_vits14",
